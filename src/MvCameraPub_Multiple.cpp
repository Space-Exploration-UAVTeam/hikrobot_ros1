...
int main(int argc, char **argv)
{
	...
	// //手动设置曝光		
	mRet = MV_CC_SetExposureTime(m_handle, 25000);//室外下午500，室内丁楼25000
	//开始采集图像
	...
	//手动设置曝光
	nRet = MV_CC_SetExposureTime(n_handle, 25000);
	//开始采集图像
	...
	while (ros::ok())
	{
		...
		nRet = MV_CC_SetExposureTime(n_handle, Exposure_Time);
		...
		// nRet = MV_CC_GetOneFrameTimeout(n_handle, nFrameBuf, nBufSize, &stInfo, 1000);
		if (mRet != 0)
		{
