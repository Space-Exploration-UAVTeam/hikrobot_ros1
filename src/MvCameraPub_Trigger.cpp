	...
	while (ros::ok())
	{
		...
		else
		{
			...
			if (stInfo.enPixelType == PixelType_Gvsp_BGR8_Packed)
			{
				...
				cv::resize(mImg, mImg, cv::Size(640, 512));
...
static void* WorkThread_2(void* n_handle)
{
	...
	{
    	//调整曝光
        nRet = MV_CC_SetExposureTime(n_handle, Exposure_Time*1.1);
		...
		else
		{
			...
			if (stInfo.enPixelType == PixelType_Gvsp_BGR8_Packed)
			{
				...
				cv::resize(nImg, nImg, cv::Size(640, 512));
...
void exposure_callback(const hikrobot::exposure::ConstPtr &msg)
{
	// cout<<"exposure_time got!"<<endl;	
...
int main(int argc, char **argv)
{
    ...
    timestamp_sub_ = n.subscribe("/mavros/cam_imu_sync/cam_imu_stamp", 1, bufferTimestamp);
...
///////////////////////////////////////////////////////////////0311
	...
	// 手动设置初始曝光		
	mRet = MV_CC_SetExposureTime(m_handle, 3200);/////////////室外下午500，室内15000
	//开始采集图像
	...
	//手动设置初始曝光
	nRet = MV_CC_SetExposureTime(n_handle, 3200);/////////////室外下午500，室内15000
	//开始采集图像
